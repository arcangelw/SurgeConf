...
| 配置方案 | 创建多套配置（日常/工作/英文），按场景切换，含 General/DNS/Rewrite/MITM
| 中英双语 | 同一份数据可生成中文或英文分组名的配置文件 |
| MITM 配置管理 | 在配置方案中管理 MITM 参数，支持手动填写 Base64 或上传 .p12 自动转换 |
...
**4. 在 SurgeConf 中配置 MITM** — 在「配置方案」页面点击 MITM 按钮展开配置面板：

- **skip-server-cert-verify** — 跳过服务端证书验证（默认开启，内网环境可关闭）
- **tcp-connection** — 对 TCP 连接进行 MITM（默认开启）
- **h2** — 对 HTTP/2 连接进行 MITM（默认开启）
- **ca-passphrase** — CA 私钥密码（可选）
- **ca-p12** — 支持两种方式：
  - **手动填写**：粘贴 .p12 证书的 Base64 编码字符串
  - **上传自动转换**：选择 .p12 文件，自动转换为 Base64 填入
- **hostname** — 需要 MITM 解密的域名列表，逗号分隔
